...
def enter_both(enter1, enter2):
    crypted_message = enter1.get()
    decoder_guess = enter2.get()
    array_matching(crypted_message,decoder_guess)
    

def array_matching():
    crypted_message=input("Please enter your cypher text (must be the output of Enigma):")
    decoder_guess=input("Please enter your plaintext (must be less than or equal to encrypted message):")
...
def update_plugs(plugboard_mapping):
    for i in range(len(plugboard_mapping)):
...
if __name__ == "__main__":
	...
	buttonCypher = Button(gui, text='Enter', fg='black', bg='light grey',
					command=lambda: enter_cypher(entered_cypher), height=2, width=7,)
	...
	buttonCrib = Button(gui, text='Enter', fg='black', bg='light grey',
					command=lambda: enter_crib(entered_crib), height=2, width=7,)
